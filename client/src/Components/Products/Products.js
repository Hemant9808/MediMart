...
const Products = () => {
  ...
  const { categoryWiseProducts } = useSelector((state) => state.product);
  const { cart } = useSelector((state) => state.CartDetails );  console.log("cart",cart);
    //const {  bc } = useSelector((state) => state.CartDetails );  console.log("cart",bc);

  

  console.log("categoryWiseProducts page", categoryWiseProducts);
